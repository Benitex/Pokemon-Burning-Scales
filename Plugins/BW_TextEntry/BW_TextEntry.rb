  ...
  def pbStartScene(helptext,minlength,maxlength,initialText,subject=0,pokemon=nil)
    ...
    when 2   # Pokémon
      if pokemon
        ...
        @sprites["gender"].x = 430 - 306
        @sprites["gender"].y = 54 - 4
        @sprites["gender"].bitmap.clear
        pbSetSystemFont(@sprites["gender"].bitmap)
        textpos = []
        if pokemon.male?
          textpos.push([_INTL("♂"), 0, -6, false, Color.new(0,0,214),Color.new(15,148,255)])
        elsif pokemon.female?
          textpos.push([_INTL("♀"), 0, -6, false,Color.new(198,0,0),Color.new(255,155,155)])
        end
    ...
    when 4   # Storage box
      ...
      charheight=@sprites["subject"].bitmap.height
      @sprites["subject"].x = 44*2 - charwidth/8
      @sprites["subject"].y = 26*2 - charheight/2 + 6
    end
...
#Fixes text colour in keyboard mode
...
class PokemonEntryScene2
  ...
  class NameEntryCursor
    ...
    def updateCursorPos
      ...
      when PokemonEntryScene2::OK   # OK
        ...
        if value >= 0
          @sprite.x = 52 + 32 * (value % PokemonEntryScene2::ROWS) - 4
          @sprite.y = 180 + 38 * (value / PokemonEntryScene2::ROWS) - 6
          @cursortype = 0
        end
      end
    end
  ...
  def pbStartScene(helptext,minlength,maxlength,initialText,subject=0,pokemon=nil)
    ...
    underline_bitmap = BitmapWrapper.new(22, 6)
    underline_bitmap.fill_rect(0,2,22,4,Color.new(80,136,152))
    underline_bitmap.fill_rect(0,0,22,4,Color.new(80,176,200))
    ...
    when 4   # Storage box
      ...
      charheight = @sprites["subject"].bitmap.height
      @sprites["subject"].x = 88 - charwidth / 8
      @sprites["subject"].y = 52 - charheight / 2 + 6
    end
  ...
  def pbDoUpdateOverlay2
    ...
    overlay.clear
    modeIcon = [[_INTL("Graphics/Pictures/Naming/icon_mode"), 44 + @mode * 62 + 4 + @mode * 2, 120, @mode * 60, 0, 60, 44]]
  ...
  def pbDoUpdateOverlay
    ...
    textPositions = [
       [@helptext, 160, 6, false, Color.new(248,248,248),Color.new(72,80,88)]
    ]
    chars = @helper.textChars
    x = 166
    for ch in chars
      textPositions.push([ch, x, 42, false, Color.new(248,248,248),Color.new(72,80,88)])
      x += 24
    end
    ...
    loop do
      ...
      if Input.trigger?(Input::SPECIAL)
        ...
        when MODE4
          ...
          pbUpdateOverlay
          #Tab switching after first charachter
          if @mode == 0 && @helper.cursor == 1
            pbChangeTab(1) if LATEST_ENTRY_MECHANICS
          end
        end
      end
    end
